  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData(result.map((item) => item?.show?.image?.original));
        setIsLoading(true);
      } catch (err) {
        console.error(err);
        setIsLoading(false);
  ...
  return (
    <div className="relative mb-12">
      ...
      <div className="relative group">
        ...
        <div
          ref={carouselRef}
          className="carousel carousel-center gap-8 flex overflow-x-hidden overflow-y-hidden flex-row w-full mt-4 bg-transparent "
        >
          {isLoading ? (
